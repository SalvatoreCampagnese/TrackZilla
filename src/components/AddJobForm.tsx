...
export const AddJobForm: React.FC<AddJobFormProps> = ({
  ...
  const predefinedTags = ['Dream Job', 'Startup', 'Importante', 'RAL Interessante', 'Referral', 'Remote Friendly', 'Grande Azienda', 'Opportunità di Crescita'];
  ...
  const FormContent = () => <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-medium ${step === 'extract' ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'}`}>
                1
              </div>
              ...
              <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-medium ${step === 'details' ? 'bg-blue-600 text-white' : 'bg-muted text-muted-foreground'}`}>
                2
              </div>
      ...
      <ScrollArea className="flex-1 pr-4">
        {step === 'extract' && <div className="space-y-4 sm:space-y-6">
            <div>
              ...
              <Textarea ref={textareaRef} id="jobDescription" value={jobDescription} onChange={e => setJobDescription(e.target.value)} placeholder="Incolla qui la job description completa..." className="mt-2 min-h-[150px] sm:min-h-[200px] bg-background border-border text-foreground resize-none" required autoFocus onFocus={e => {
        ...
        {step === 'details' && <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            <div className="space-y-4 p-3 sm:p-4 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
              ...
              <div className="space-y-4">
                ...
                }))} className="mt-1 bg-background border-border text-foreground" placeholder="es. 45k, ND" />
                  </div>
                ...
                <div>
                  ...
                  <select id="status" value={status} onChange={e => setStatus(e.target.value as JobStatus)} className="mt-1 w-full px-3 py-2 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-background text-foreground">
              ...
              }))} className="mt-1 bg-background border-border text-foreground resize-none" rows={3} />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              {tags.length > 0 && <div className="flex flex-wrap gap-2">
                  {tags.map((tag, index) => <div key={index} className="inline-flex items-center gap-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 px-2 py-1 rounded-full text-xs sm:text-sm">
                      <Tag className="w-3 h-3" />
                      {tag}
                      <button type="button" onClick={() => handleRemoveTag(tag)} className="ml-1 text-blue-600 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-100">
                        <X className="w-3 h-3" />
                      </button>
                    </div>)}
                </div>}
              ...
              <div className="flex gap-2">
                <Input value={newTag} onChange={e => setNewTag(e.target.value)} placeholder="Aggiungi tag personalizzato..." className="flex-1 bg-background border-border text-foreground text-sm" onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), handleAddCustomTag())} />
                <Button type="button" onClick={handleAddCustomTag} variant="outline" size="sm" disabled={!newTag.trim()} className="text-foreground border-border hover:bg-accent px-3">
      ...
      <div className="flex-shrink-0 pt-4 sm:pt-6 border-t mt-4 sm:mt-6">
        {step === 'extract' ? <div className="space-y-3">
            <Button type="button" onClick={handleParseJob} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-10 sm:h-11" disabled={!jobDescription.trim()}>
              <Wand2 className="w-4 h-4 mr-2" />
              Estrai Dati e Continua
            </Button>
            {isMobile && <Button type="button" variant="outline" onClick={handleClose} className="w-full text-foreground border-border hover:bg-accent h-10">
                Annulla
              </Button>}
          </div> : <div className="flex flex-col sm:flex-row gap-3">
            <Button type="button" variant="outline" onClick={() => setStep('extract')} className="flex-1 text-foreground border-border hover:bg-accent h-10 sm:h-11 order-2 sm:order-1">
              Indietro
            </Button>
            <Button onClick={handleSubmit} className="bg-blue-600 hover:bg-blue-700 text-white flex-1 h-10 sm:h-11 order-1 sm:order-2">
  ...
  return <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent onOpenAutoFocus={e => e.preventDefault()} onPointerDownOutside={e => e.preventDefault()} onInteractOutside={e => e.preventDefault()} className="max-w-2xl w-full max-h-[90vh] flex flex-col p-6 overflow-y-scroll">
